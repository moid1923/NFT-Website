...
import BidPlacement from "./BidPlacement"; // Import the BidPlacement modal
...
function Oction({ maxImages = 12, title = "Auction", bid = "Place a bid", explore = "", showButtons = false }) {
  ...
  return (
    <div className="py-10 lg:my-10">
      <div className="max-w-[1320px] mx-auto px-4 z-10">
          ...
          {oction.slice(0, maxImages).map((nft) => (
            ...
            >
              <div className="p-3 flex items-center space-x-3">
                <img src={hero} className="w-[53px] h-[53px] rounded-full" alt="Owner" />
                <div>
              ...
              <div className="relative max-w-[292px] h-[314px] flex justify-center items-center">
                <img src={nft.img} alt="NFT" className="w-[100%] h-[100%] object-cover absolute rounded-2xl" />
                ...
                {showButtons && (
                  <div className="p-3 w-full flex justify-between items-center absolute bottom-1.5 z-40">
                    ...
                    <span className="text-sm font-bold w-[123px] h-[36px] bg-white px-3 py-[11px] text-[12px] rounded-lg flex justify-center items-center gap-1">
                      Price:
                      <span className="w-[15px] h-[15px] rounded-full bg-black flex items-center justify-center">
        ...
        {explore && (
          <button onClick={() => console.log("Explore Clicked")}
            className="flex text-[22px] lg:w-[228px] lg:h-[58px] gap-2 mx-auto cursor-pointer bg-gradient-custom text-white px-8 py-3 rounded-lg shadow-md hover:opacity-70 transition duration-200 text-center items-center mt-[70px]">
      ...
      {/* Bid Placement Modal */}
      <BidPlacement isOpen={isModalOpen} onClose={handleCloseModal} />
    </div>
  );
}
